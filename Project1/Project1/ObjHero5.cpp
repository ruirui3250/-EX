...
//位置情報X取得用
float CObjHero5::GetX()
{
	return m_x;
}

//位置情報Y取得用
float CObjHero5::GetY()
{
	return m_y;
}
...
void CObjHero5::Action()
{
	...
	if (Input::GetVKey(VK_SPACE) == true)
	{
		if (m_la >= 0)
		{
			...
			m_la--;
			//弾丸オブジェクト作成
			CObjSitaLaserBullet* obj_b2 = new CObjSitaLaserBullet(m_x + 3.0f, m_y -= 0.0f);//弾丸オブジェクト
			Objs::InsertObj(obj_b2, OBJ_SITA_LASER_BULLET, 100);//作った弾丸オブジェクト
			m_la--;
			//弾丸オブジェクト作成
			CObjTateLaserBullet* obj_b3 = new CObjTateLaserBullet(m_x + 3.0f, m_y -= 0.0f);//弾丸オブジェクト
			Objs::InsertObj(obj_b3, OBJ_TATE_LASER_BULLET, 100);//作った弾丸オブジェクト
			m_la--;
		}
	...
	if (m_time == 3400)//時間経過により弾回復
	{

		m_ka = 3;//拡散弾丸
	}
	if (m_time == 3800)//時間経過により弾回復
	{

		m_ka = 3;//拡散弾丸
	}
	if (m_time == 4200)//時間経過により弾回復
	{

		m_ka = 3;//拡散弾丸
	}
	if (m_time == 4400)//時間経過により弾回復
	{
