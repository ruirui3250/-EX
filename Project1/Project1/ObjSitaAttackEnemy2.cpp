...
//コンストラクタ
CObjSitaAttackEnemy2::CObjSitaAttackEnemy2(float x, float y)
{
	m_x = x;
	m_y = y;
}
...
//イニシャライズ
void CObjSitaAttackEnemy2::Init()
{
	m_hp = 15;
	m_vx = 0.0f;
	m_vy = 0.0f;
	...
	//当たり判定用HitBoxを作成.
	Hits::SetHitBox(this, m_x, m_y, /*横長*/32,/*縦長*/ 32, ELEMENT_ENEMY, OBJ_SITA_ATTACK_ENEMY2, 1);

}
//アクション
void CObjSitaAttackEnemy2::Action()
{
	m_time++;//1加算
	...
	if (m_time > 50)
	{
		...
		Objs::InsertObj(obj_b, OBJ_SITA_BULLET_ENEMY, 50);//弾丸敵機object登録
	}
	//移動方向
	m_vx = 0.0f;
	m_vy = -1.0f;
...
//ドロー
void CObjSitaAttackEnemy2::Draw()
{
	//描画カラー情報
